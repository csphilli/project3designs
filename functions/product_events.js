  ...
  try {
    ...
    switch (stripeEvent.type) {
        ...
        const { error } = await supabase.from("products").insert([
          {
            ...
            project_link: product.metadata.project_link,
      ...
      case HOOKS.PRODUCT_UPDATED: {
        const price_data = await stripe.prices.retrieve(product.default_price);
        ...
        if (error) {
          throw new Error(`Could not update row for ${product.id}: ${error}`);
        }
        break;
      }
      case HOOKS.PRODUCT_DELETED: {
        ...
        if (error) {
          console.error(error);
          throw new Error(`Could not delete product: ${product.id}: ${error}`);
        }
        break;
      }
